...
def query(point,mesh):
    ...
    jl_idx = jl.FEval.query(point,mesh.tree)
    return jl_idx-1
...
def evaluate(point,field,mesh):
    ...
    """
    if point.ndim == 2:
        return np.array(jl.FEval.evaluate(point[:,:2],field,mesh.tree))
    return np.array(jl.FEval.evaluate(point,field,mesh.tree))
