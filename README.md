...
This particular branch moves some calculations into Julia; the Julia install is handled automatically. Meshing is still done in Python. Currently, this version solves the circular fiber waveguide example from the doc around 40% faster than base wavesolve. The vectorial solver is around $4\times$ faster judging from the doc examples.
